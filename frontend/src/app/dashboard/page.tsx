...
export default function DashboardPage() {
  ...
  return (
    <div>
      ...
      {/* Display labs summary */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-10">
        {labOptions.map((lab) => (
          <div key={lab} className="bg-blue-50 shadow rounded p-4">
            <h2 className="font-semibold text-lg mb-1">{lab}</h2>
            <p className="text-sm text-gray-600">
              Total Items: {countItems(lab)}
            </p>
            <p className="text-sm text-gray-600">
              Total Produk: {countProducts(lab)}
            </p>
          </div>
        ))}
      </div>
